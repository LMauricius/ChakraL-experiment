...
def outputExprProductions(filenameIn: str, filenameOut: str):
	...
	try:
		...
		for lStrInd in meaningfulLines(filenameIn):
			l = lStrInd[0]
			if l[0] == '@':
				prevExpr = expr
				expr = l[1:]
				...
				if optype == 'BINARY':
					...
					if assoc == '>':
						...
						raise FormatError("RUNARY can't be <")
				elif optype == 'LUNARY':
					if assoc == '<':
						LN(name + " = " + operator + " <ONL> <val:" + expr + ">")
					else:
						raise FormatError("LUNARY can't be >")
				else:
					raise FormatError("Expected BINARY or RUNARY or LUNARY")
