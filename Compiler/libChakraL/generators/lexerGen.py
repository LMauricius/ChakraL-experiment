...
def writeLexerH(lexer: Lexer, filename: str):
    ...
    TB();TB();LN('')
    TB();TB();LN('std::wstring fullname() const;')
    TB();LN("};")
    LN("")
...
def writeLexerCPP(lexer: Lexer, filename: str, headerfile: str, useSingleDFA: bool):
    ...
    TB();LN("};")
    LN("")
    TB();TB();LN('std::wstring Token::fullname() const {')
    TB();TB();TB();LN('std::wstringstream ss;')
    TB();TB();TB();LN('ss << WTokenNames[(int)type] << ":" << line << ":" << character << ":";')
    TB();TB();TB();LN('for (auto c : str) {')
    TB();TB();TB();TB();LN("if (!isprint(c) || c == '\\'' || c == '\\\"' || c == '\\n') {")
    TB();TB();TB();TB();TB();LN('ss << L"\\\\u" << std::setfill(L\'0\') << std::setw(4) << std::hex << (int)c;')
    TB();TB();TB();TB();LN("} else {")
    TB();TB();TB();TB();TB();LN("ss << c;")
    TB();TB();TB();TB();LN("}")
    TB();TB();TB();LN('}')
    TB();TB();TB();LN('return ss.str();')
    TB();TB();LN('}')
    LN("")
    TB();LN("std::list<Token> tokenize(std::wstring &input, std::list<LexerError>& outErrors)")
    TB();LN("{")
